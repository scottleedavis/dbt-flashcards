...
let CardPage = class CardPage {
    constructor(state, sanitizer) {
        ...
        this.index = 0;
        this.flipped = false;
        const randomizedData = this.shuffle(_card_data__WEBPACK_IMPORTED_MODULE_4__["data"]);
        ...
        state.subject.subscribe((f) => {
            if (f === "next") {
                this.index = this.index + 1 >= _card_data__WEBPACK_IMPORTED_MODULE_4__["data"].length ? 0 : this.index + 1;
            }
            else if (f === "previous") {
